            ...
            <div className="mt-2">
                {lections.length > 0 ? (
                    lections.map((lection) => (
                        <div key={lection.id} className="bg-white border border-black px-6 py-3 flex flex-col lg:flex-row justify-between w-full hover:bg-slate-100 items-center">
                            ...
                            <div className="flex gap-3 mt-4 lg:mt-0">
                                <Link href={"/lections/edit/"+lection.id} className="cursor-pointer lg:mx-2 px-4 py-2 border border-sky-500 rounded-sm bg-sky-500 text-white hover:bg-sky-600">Editar</Link>
                                <button onClick={() => {deleteLection(lection.id)}} className="cursor-pointer lg:mx-2 px-4 py-2 border border-red-500 rounded-sm bg-red-500 text-white hover:bg-red-600">Borrar</button>
                            </div>
                        </div>
                    ))
